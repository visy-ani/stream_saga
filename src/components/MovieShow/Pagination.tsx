...
const Pagination: React.FC<PaginationProps> = ({
  ...
  const getVisiblePaginationDots = () => {
    if (totalItems <= 7) {
      // If total items are less than or equal to 7, show all dots
    ...
    if (startDot > 1) {
      // If there's a gap between the first dot and the start of our visible range
      items.push({ index: -1, type: "ellipsis" });
    }

    for (let i = startDot; i <= endDot; i++) {
      // Add dots around current
      items.push({ index: i, type: "dot" });
    }

    if (endDot < totalItems - 2) {
      // If there's a gap between the end of our visible range and the last dot
  ...
  return (
    <>
      {/* Desktop Controls */}
      <div className="hidden md:flex items-center justify-between mt-12 w-full">
        <button
          onClick={prevMovie}
        ...
        <div className="flex items-center gap-2 px-4 max-w-sm overflow-x-auto scrollbar-hide">
